...
function timeSpanToString(startDate, endDate) {
  ...
  let str = '';
  str += '00'.concat(hours).slice(-2).concat(':');
  str += '00'.concat(minute).slice(-2).concat(':');
  str += '00'.concat(seconds).slice(-2).concat('.');
  str += '000'.concat(ms).slice(-3);
  return str;
}
...
function angleBetweenClockHands(date) {
  ...
  let hours = tmpDate.getHours() - 3;
  if (hours >= 12) hours -= 12;
  const minute = tmpDate.getMinutes();
  const hoursangle = hours * 30 + minute * 0.5;
  const minangle = minute * 6;
  let abs = Math.abs(hoursangle - minangle);
  if (abs > 180) abs = 360 - abs;
  if (abs === 0) return 0;
  return (abs / 180) * Math.PI;
}
